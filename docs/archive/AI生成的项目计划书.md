...
> **注意：本文为旧稿归档，禁止作为实现依据。**  
> 其中 `USE_MOCK` / `mock_task_runner` / `start_mock_task` / 终态 `completed` 等已废弃。  
> 现行方案：独立 **imggen-stub** 走真实 MQ 链路 → [imggen-stub联调模式设计.md](../details/imggen-stub联调模式设计.md)  
> 日常开发入口 → [README.md](../README.md) / [代码阅读指南.md](../代码阅读指南.md)  
> 差异对照 → [归档与现行规范差异.md](./归档与现行规范差异.md)
...
| `.env.example` | 环境变量示例 |
| `config.py` | 后端地址、WebSocket 地址 |
| `constants.py` | 工具列表、工作流模板、算力类型 |
| `models.py` | 任务模型、状态枚举、表单模型 |
| `store.py` | 本地任务状态存储（Gradio 时代） |
...
> **以下为旧稿环境变量摘录，已废弃，禁止复制。**  
> 现行后端 `.env` 见 [部署与环境变量.md](../details/部署与环境变量.md)；本地假生图用 imggen-stub。
...
> **旧稿状态枚举，已废弃。**  
> 现行终态为 `succeeded` / `failed` / `canceled`（不用 `completed`）。  
> 见 [任务状态与任务流转.md](../details/任务状态与任务流转.md)。

```text
# 旧稿（勿用）：created / queued / dispatching / running / completed / failed / cancelled / retrying
# 现行：       created / dispatching / queued / running / succeeded / failed / canceled
```
...
# 旧稿枚举已删除。现行见 docs/details/任务状态与任务流转.md（终态 succeeded，非 completed）
...
# USE_MOCK 已废弃 — 见 imggen-stub联调模式设计.md
...
def list_tasks(filter_type: str = "全部") -> List[GenerationTask]:
    ...
    if filter_type == "已完成":
        return [task for task in tasks if task.status == TaskStatus.SUCCEEDED]
...
> 旧稿状态字段原为 `"completed"`，现行统一为 `"succeeded"`。
...
> **旧稿 Gradio `websocket_client.py` 已废弃，禁止复制。**  
> 旧稿曾映射 `success/completed` → `COMPLETED`；现行终态为 `succeeded`。  
> 现行 Vue WS → [WebSocket推送示例.md](../examples/WebSocket推送示例.md)

```python
# 旧稿 _normalize_status / start_websocket_listener 代码块已删除。
# imggen callback 字段 status="success" → 业务任务 status="succeeded"
```

---

# 17. Mock Task Runner（已废弃）

> **本节为旧稿，业务侧 `mock_task_runner` / `start_mock_task` 已废弃，禁止复制。**  
> 现行方案：独立 **imggen-stub** 消费 task topic、投递 callback topic。  
> 文档 → [imggen-stub联调模式设计.md](../details/imggen-stub联调模式设计.md)  
> 示例 → [imggen-stub示例.md](../examples/imggen-stub示例.md)
...
> **以下为旧稿 Gradio 启动代码摘录。** `USE_MOCK` 已废弃，禁止复制到现行 FastAPI 实现。

```python
# 旧稿入口已删除可运行片段。现行启动见 docs/开发入门.md
# Gradio 时代：build_app().launch(...)
```
