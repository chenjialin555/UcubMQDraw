...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/v1/auth/register` | 注册（手机号或邮箱 + 密码） |
| POST | `/api/v1/auth/login` | 登录 |
| POST | `/api/v1/auth/guest` | 游客 token（可选） |
...
| GET | `/api/v1/images/generations/tasks` | 任务列表（分页，当前用户） |
| GET | `/api/v1/images/generations/tasks/{taskId}` | 单条任务 |
| DELETE | `/api/v1/images/generations/tasks/{taskId}` | 软删除任务 |
| POST | `/api/v1/images/generations/tasks/{taskId}/rerun` | 再次生成 |
| POST | `/api/v1/images/generations/tasks/{taskId}/cancel` | 取消任务 |
| PATCH | `/api/v1/images/generations/tasks/{taskId}/favorite` | 收藏 |
| WS | `/api/v1/ws/tasks?token=<jwt>` | 任务事件推送 |

设计细节 → [details/用户与鉴权设计.md](./details/用户与鉴权设计.md)
...
响应：`{ "success": true }`。第一版前端删除 `localStorage.access_token` 并断开 WS 即可。
...
鉴权详见 [details/用户与鉴权设计.md](./details/用户与鉴权设计.md)。
...
> 消息格式与事件类型权威定义见 [API接口.md](../API接口.md#十一websocket)。
> 鉴权与用户隔离见 [用户与鉴权设计.md](./用户与鉴权设计.md)。
...
角色权限 / 组织团队
实名认证
refreshToken（可选，第一版可只用 7 天 accessToken）
```

> 不做验证码 = 只能证明「用户填了这个号/邮箱」，不能证明归属。表字段保留 `phone_verified` / `email_verified` 默认 `false`，后续接验证码不用改表。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/v1/auth/register` | 注册（account + password） |
| POST | `/api/v1/auth/login` | 登录 |
| POST | `/api/v1/auth/guest` | 游客 token |
| GET | `/api/v1/auth/me` | 当前用户 |
| PATCH | `/api/v1/auth/me` | 修改昵称 |
| POST | `/api/v1/auth/password/change` | 修改密码（游客不可用） |
| POST | `/api/v1/auth/logout` | 退出（前端删 token） |
...
```text
1. users 表 + guest 初始化
2. password hash + JWT 工具
3. /auth/register、/login、/guest、/me
4. 前端 authStore + http 拦截器
5. generation_tasks.user_id + 任务接口接 current_user
6. WS token 鉴权 + send_to_user
7. 游客 localStorage taskId 过滤
8. 修改昵称 / 修改密码 / 退出
```
...
> 鉴权见 [用户与鉴权设计.md](../details/用户与鉴权设计.md)。
...
export function addGuestTaskId(taskId: string) {
  const ids = new Set(getGuestTaskIds())
  ids.add(taskId)
  localStorage.setItem(KEY, JSON.stringify([...ids]))
}

export function shouldAcceptGuestTask(taskId: string): boolean {
  return getGuestTaskIds().includes(taskId)
}
```

`createTask` 成功后若是游客：`addGuestTaskId(task.taskId)`。
...
app.mount('#app')
bootstrapApp()  // auth → WS → fetchTasks（非游客）
```
...
## 八、authStore 与 bootstrap

```ts
// stores/authStore.ts（摘要）
import { defineStore } from 'pinia'
...
export const useAuthStore = defineStore('auth', {
  ...
  getters: {
    isLoggedIn: state => !!state.user && state.user.userType !== 'guest',
...
详见 [用户与鉴权设计.md](../details/用户与鉴权设计.md)。
