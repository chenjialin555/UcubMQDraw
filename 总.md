...
> 由 merge-docs.bat 自动生成，请勿手工编辑。
> 生成时间：2026-07-13 00:05:36
> 共 28 个文件
...
main.ts 启动 connectTaskWebSocket(?token=...)
  ...
  → 游客仅展示 localStorage guest_task_ids
...
phone_verified / email_verified 默认 0
...
# USE_MOCK ? start_mock_task : mq_service.send(task_dispatch_params, tool_key)
```
...
<!-- 来源: docs/archive/features-to-tools术语对照.md -->
...
| 任务队列 | Redis List 等备选 | RocketMQ | **队列只用 RocketMQ** |
| Redis Pub/Sub | 常与 WS 混写 | 仅多实例 WS（Phase 3） | MVP 不做 |
